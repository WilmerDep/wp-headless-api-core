(function ($) {
	...
	function initMediaPicker() {
		$(document).on('click', '.headless-services-select-image', function (event) {
			...
			var frame = wp.media({ title: text('chooseImage', 'Seleccionar imagen del servicio'), button: { text: text('useImage', 'Usar esta imagen') }, library: { type: 'image' }, multiple: false });
	...
	function initOrdering() {
		$('.headless-services-sortable').each(function () {
			var $list = $(this);
			$list.sortable({ handle: '.headless-directory-drag-handle', placeholder: 'ui-sortable-placeholder', forcePlaceholderSize: true, update: function () {
				...
				$.post(cfg.ajaxUrl, { action: 'headless_services_save_order', nonce: cfg.orderNonce, ids: ids, orderType: orderType }).done(function (response) {
					...
					} else { setSaveState($state, 'error', (response && response.data && response.data.message) || text('error', 'No se pudo guardar.')); }
	...
	function initDropzone() {
		...
		$drop.on('dragenter dragover', function (event) { event.preventDefault(); $drop.addClass('is-dragover'); }).on('dragleave dragend drop', function (event) { event.preventDefault(); $drop.removeClass('is-dragover'); });
	...
	function renderPreview(data) {
		...
		$('.headless-directory-import-kpis').html('<div class="headless-directory-import-kpi is-ready"><strong>' + summary.ready + '</strong><span>Listos</span></div><div class="headless-directory-import-kpi is-warning"><strong>' + summary.warning + '</strong><span>Con advertencias</span></div><div class="headless-directory-import-kpi is-error"><strong>' + summary.error + '</strong><span>Con errores</span></div>');
		$('.headless-services-import-preview table thead tr').html('<th>#</th><th>Servicio</th><th>Departamento</th><th>Grupos</th><th>Inicio</th><th>Requisitos</th><th>Estado</th><th>Validación</th>');
		var html = '';
		(data.rows || []).forEach(function (row) {
			...
			var featured = parseInt(row.featured || 0, 10) === 1 ? 'Sí · orden ' + parseInt(row.featured_order || 0, 10) : 'No';
	...
	function validateImport() {
		...
		formData.append('action', 'headless_services_preview_import'); formData.append('nonce', cfg.importNonce); formData.append('file', importState.file);
	...
	function mergeBatchResult(batch) { ['created', 'updated', 'skipped', 'failed'].forEach(function (key) { importState.result[key] += batch && batch[key] ? parseInt(batch[key], 10) : 0; }); }

	function importBatch(offset) {
		return $.post(cfg.ajaxUrl, { action: 'headless_services_import_batch', nonce: cfg.importNonce, token: importState.token, offset: offset, batchSize: 10, mode: $('#headless-services-import-mode').val() || 'create_only', importImages: $('.headless-services-import-images').is(':checked') ? 1 : 0 }).then(function (response) {
			if (!response || !response.success) { return $.Deferred().reject(response).promise(); }
			var data = response.data || {}; mergeBatchResult(data.batchResult || {});
			var total = data.total || importState.total || 1;
			var progress = Math.min(100, Math.round(((data.offset || 0) / total) * 100));
			$('.headless-directory-progress-bar span').css('width', progress + '%'); $('.headless-directory-progress-label').text(progress + '%');
			if (data.done) {
	...
	function runImport() {
		...
		$('.headless-directory-progress').prop('hidden', false); $('.headless-directory-progress-bar span').css('width', '0%'); $('.headless-directory-progress-label').text('0%'); $('.headless-directory-import-result').empty();
	...
	$(function () {
		initMediaPicker(); initRequirements(); initOrdering(); initDropzone();
